...
async def ingest_diet_data(
    request: DietIngestRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
):
    """Ingest diet/macro data and store in diet table"""
    try:
        ...
        for data_point in request.data:
            try:
                ...
                logger.info(f"Added diet record for {meal_datetime}")

            except Exception as e:
                logger.error(f"Error processing diet data point: {str(e)}")
                continue

        # Commit all changes
        db.commit()

        logger.info(f"Successfully ingested {inserted_records} diet records")
        ...
        logger.error(f"Error ingesting diet data: {str(e)}")
        db.rollback()
        raise HTTPException(
...
@router.get("/diet", response_model=DietExportResponse)
...
async def add_diet_record(
    ...
    try:
        ...
        logger.error(f"Error adding diet record: {str(e)}")
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.delete("/diet/record/{record_id}", response_model=DietDeleteResponse)
async def delete_diet_record(
    ...
    try:
        ...
        logger.error(f"Error deleting diet record: {str(e)}")
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.get("/diet/daily/{date}", response_model=DailyAggregationResponse)
async def get_daily_diet_records(
    date: str,  # Format: YYYY-MM-DD
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
):
    """Get all diet records for a specific day"""
    try:
        # Parse the date
        ...
        logger.info(f"Retrieved {len(records)} diet records for {date}")
        ...
        logger.error(f"Error fetching daily diet records: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.get("/diet/aggregate", response_model=DailyAggregationResponse)
async def get_diet_aggregations(
    ...
    try:
        ...
        logger.error(f"Error fetching diet aggregations: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
